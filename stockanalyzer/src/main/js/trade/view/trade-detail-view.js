 ...
 export default function TradeDetailView({ itemState, appPrefs, onOption }) {
   ...
   if (
     ...
     for (let i = 0; i < itemState.item.tradeDetails.length; i++) {
       ...
       cells.push(<td key="FILLPRICE">{itemState.item.tradeDetails[i].assetPrice}</td>);
       cells.push(<td key="ORDERCONDITION">{itemState.item.tradeDetails[i].orderCondition}</td>);
